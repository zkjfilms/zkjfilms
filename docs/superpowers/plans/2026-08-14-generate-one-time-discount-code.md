# Generate + One-Time Use Discount Code Shortcuts Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add three small, client-side-only shortcuts to the discount-code admin form: a button that generates a random readable code, a copy-to-clipboard button, a checkbox that prefills the usage limit to 1, and a brief confirmation panel showing the new code right after it's created.

**Architecture:** All changes live in `app/admin/discount-codes/DiscountCodeForm.tsx`. No new files, no new API routes, no schema changes — the existing create/edit `POST`/`PATCH` endpoints, validation, and the atomic redemption-limit enforcement are untouched. The random code generator and the copy button are pure client-side helpers with no server round-trip.

**Tech Stack:** Same as the existing admin UI — no new dependencies. `crypto.getRandomValues` (native browser API) and `navigator.clipboard.writeText` (native browser API).

## Global Constraints

- Spec source: `docs/superpowers/specs/2026-08-14-generate-one-time-discount-code-design.md`. Read it before starting.
- The generated code charset excludes visually-confusable characters: no `0`/`O`, `1`/`I`/`L`. That leaves 23 letters (A-Z minus I, O, L) + 8 digits (2-9) = 31 characters. Code length is 8 characters.
- The "One-time use" checkbox is a one-shot prefill, not a synced/locked control: checking it sets the Usage limit field to `"1"`; unchecking it does nothing to that field (no clearing, no special behavior) — the field remains freely editable either way, exactly as it is today.
- The post-creation confirmation panel appears only after a successful **create** (not edit). Editing an existing code still closes and refreshes immediately, unchanged.
- No test framework exists in this codebase. Verification is `npx tsc --noEmit && npx eslint app lib components`, an isolated `node -e` check of the code-generation logic, and a manual browser walkthrough. The browser walkthrough involves real button clicks and clipboard interaction that a subagent's environment may not reliably support (per this plan's own prior work in this codebase) — the implementer should verify what's programmatically checkable and do a careful self-read of the JSX/state logic; the controller will do a final interactive click-through in the browser after this task's review lands.

---

## Task 1: Generate, copy, one-time-use checkbox, and post-creation confirmation

**Files:**
- Modify: `app/admin/discount-codes/DiscountCodeForm.tsx`

**Interfaces:**
- None consumed from or produced for other tasks — this is the only task in the plan, entirely self-contained within one file.

- [ ] **Step 1: Replace the full file**

Replace the full file `app/admin/discount-codes/DiscountCodeForm.tsx` with:
...
Expected: no errors (2 pre-existing `components/Navbar.tsx` errors, unrelated to this file, are known baseline noise).

- [ ] **Step 3: Verify — code-generation logic in isolation**

```bash
node -e '
const CODE_CHARSET = "ABCDEFGHJKMNPQRSTUVWXYZ23456789";
console.log("charset length:", CODE_CHARSET.length); // expect 31
console.log("has 0/O/1/I/L:", /[0O1IL]/.test(CODE_CHARSET)); // expect false
...
for (let i = 0; i < 5; i++) {
  ...
  console.log(code, code.length === 8 && [...code].every((c) => CODE_CHARSET.includes(c)));
}
'
```

Expected: charset length `31`, `has 0/O/1/I/L: false`, and each of the 5 generated codes prints alongside `true` (correct length, every character from the allowed set).

- [ ] **Step 4: Self-review the JSX/state logic**

Since clipboard interaction and button clicks aren't verifiable without a real browser in this environment, read back through the diff carefully and confirm by inspection:
- `handleGenerateCode` only ever updates `form.code`, nothing else.
- `handleOneTimeUseChange`: checking sets `maxRedemptions` to `"1"`; unchecking updates only `oneTimeUse`'s own state, leaving `form.maxRedemptions` untouched (re-read the function — there should be no `else` branch that clears or resets the field).
- The `createdCode` early-return only triggers when `!isEditing` and the POST succeeded — confirm the `isEditing` branch in `handleSubmit`'s success path still calls `onDone()` immediately with no confirmation panel, exactly as before this change.
- `CopyButton`'s `disabled={!value}` means the copy button next to the Code field is disabled until a code exists (typed or generated) — confirm this reads correctly in the JSX.

- [ ] **Step 5: Commit**

```bash
git add app/admin/discount-codes/DiscountCodeForm.tsx
git commit -m "Add generate, copy, and one-time-use shortcuts to the discount code form"
```
